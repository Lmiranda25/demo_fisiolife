...
export default function SectionTitle({ eyebrow, title, subtitle, align = 'center', light }: Props) {
  return (
    ...
    >
      ...
      {subtitle && (
        <p className={`mt-4 text-base leading-relaxed ${light ? 'text-navy-50/85' : 'text-ink/65'}`}>
          {subtitle}
        </p>
      )}
      {align === 'center' && <div className="mx-auto mt-5 h-1 w-16 rounded-full bg-leaf-gradient" />}
    </motion.div>
  )
}
