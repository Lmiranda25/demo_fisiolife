  ...
  return (
    <section className="relative flex min-h-[40vh] items-center overflow-hidden pt-20">
      <div className="absolute inset-0 -z-10 bg-navy-900">
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-navy-900/92 to-navy-800/75" />
      </div>
      <div className="container-pro py-12">
        <motion.nav
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          className="flex items-center gap-1.5 text-xs text-navy-50/70"
        >
          <Link to="/" className="hover:text-leaf-300">
            Inicio
          </Link>
          <ChevronRight className="h-3.5 w-3.5" />
          <span className="text-leaf-300">{crumb}</span>
        </motion.nav>
        <motion.div initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          {eyebrow && <span className="eyebrow mt-4 block text-leaf-300">{eyebrow}</span>}
          <h1 className="mt-2 max-w-2xl font-display text-3xl font-extrabold leading-tight text-white sm:text-4xl lg:text-5xl">
            {title}
          </h1>
          {subtitle && <p className="mt-4 max-w-xl text-base text-navy-50/85 sm:text-lg">{subtitle}</p>}
        </motion.div>
      </div>
    </section>
  )
